-- Secure Token Storage Schema with Simple City Detection
-- This schema provides secure storage for API tokens with automatic city detection
...
CREATE TABLE IF NOT EXISTS secure_tokens (
    ...
    -- Detected cities from Hostaway API (automatically populated)
    cities TEXT[] DEFAULT '{}', -- Array of cities this token has access to
...
CREATE INDEX idx_secure_tokens_cities ON secure_tokens USING GIN(cities);
...
-- Function to update token cities after validation
CREATE OR REPLACE FUNCTION update_token_cities(
    p_token_id UUID,
    p_cities TEXT[]
)
RETURNS VOID AS $$
...
-- Function to get token for a specific city
...
BEGIN
    ...
    WHERE st.token_key = p_token_key
        ...
        AND (
            -- Token has access to this specific city
            p_city = ANY(st.cities)
            -- Or token has no cities yet (needs validation)
            OR array_length(st.cities, 1) IS NULL
            -- Or for non-Hostaway tokens (global access)
            OR st.token_type != 'hostaway'
        )
    ORDER BY 
        CASE 
            WHEN p_city = ANY(st.cities) THEN 0  -- Prefer tokens with this city
...
GRANT EXECUTE ON FUNCTION update_token_cities TO authenticated;

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Token management schema created successfully!';
END $$;